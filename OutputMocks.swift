...
final class LoginApiProtocolMock: LoginApiProtocol {
    ...
    private(set) var singinCallCount = 0
    var singinHandler: ((LoginApiRequest) async throws -> (LoginApiResponse))?
    func singin(params: LoginApiRequest) async throws -> LoginApiResponse {
        singinCallCount += 1
        if let singinHandler = singinHandler {
            return try await singinHandler(params)
        }
        fatalError("singinHandler returns can't have a default value thus its handler must be set")
    }
}
...
final class UserRepositoryProtocolMock: UserRepositoryProtocol {
    ...
    private(set) var singinCallCount = 0
    var singinHandler: ((String, String) async throws -> ())?
    func singin(userName: String, password: String) async throws  {
        singinCallCount += 1
        if let singinHandler = singinHandler {
            try await singinHandler(userName, password)
        }
